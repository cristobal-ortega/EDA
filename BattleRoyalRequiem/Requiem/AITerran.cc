...
#define NAMEPLAYER TerranPortugal
...
namespace NAMEPLAYER {                        // Do not change this line
    ...
    struct AIstruct: public AIbase {          // Do not change this line
		...
		int agrupar(int id){
			...
			int x=r.i;
			int y=r.j;
			if(color(x-1,y-1) != -1 and not existeix(x-1,y-1)){
				return 6;
			}
			else if(color(x-1,y) !=-1 and not existeix(x-1,y)){
				return 7;
			}
			else if(color(x,y-1) != -1 and not existeix(x,y-1)){
				return 5;
			}
			return 9;
		}
        ...
        void fes_torn() {
            ...
            for (int i = 0; i < (int)cav.size(); ++i) {
				...
				int aux;
					...
					if( (int)cav.size() >= 40) atacar=true;
					if(torn_actual()>200 or atacar) aux = bfs2(cav[i],0,50);
					else aux = bfs2(cav[i],0,23);
					ordena(r.id, aux);
