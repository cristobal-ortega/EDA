        ...
        void fes_torn() {
            ...
            for (int i = 0; i < (int)cav.size(); ++i) {
				...
				int aux;
					...
					if(r.vida<200) aux=retirada(r.id);
					else if(torn_actual()>240 or atacar) aux = bfs2(cav[i],0,50);
					else{
						aux = bfs2(cav[i],0,23);
						if(aux==9) aux=random(9)+1;
					}
					ordena(r.id, aux);
